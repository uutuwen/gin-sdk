...
import (
	...
	"github.com/uutuwen/gin-sdk/config/source"
)
...
func TestEnvvar_Prefixes(t *testing.T) {
	...
	os.Setenv("GIN_SDK_REGISTRY", "mdns")

	var prefixtests = []struct {
		prefixOpts   []source.Option
		expectedKeys []string
	}{
		{[]source.Option{WithPrefix("APP", "GIN_SDK")}, []string{"app", "gin_sdk"}},
		{[]source.Option{WithPrefix("GIN_SDK"), WithStrippedPrefix("APP")}, []string{"database", "gin_sdk"}},
		{[]source.Option{WithPrefix("GIN_SDK"), WithStrippedPrefix("APP")}, []string{"database", "gin_sdk"}},
	}
...
func TestEnvvar_WatchNextNoOpsUntilStop(t *testing.T) {
	src := NewSource(WithStrippedPrefix("GIN_SDK_"))
	w, err := src.Watch()
	if err != nil {
		t.Error(err)
	}
